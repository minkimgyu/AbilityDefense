...
namespace PathfinderForTilemap
{
    ...
    public class AStarPathfinder : MonoBehaviour, IPathfinder
    {
        ...
        List<Vector3> _pathToDebug;
        List<Vector3> _openListToDebug;
        List<Vector3> _closedListToDebug;

        public List<Vector3> PathToDebug { get => _pathToDebug; }
        public List<Vector3> OpenListToDebug { get => _openListToDebug; }
        public List<Vector3> ClosedListToDebug { get => _closedListToDebug; }
        ...
        public void Initialize(IPathGrid<AStarPathNode> gridComponent)
        {
            ...
            _pathToDebug = new List<Vector3>();
            _openListToDebug = new List<Vector3>();
            _closedListToDebug = new List<Vector3>();

            _drawer = GetComponent<AStarPathDrawer>();
            _drawer.Initialize(this);
        }

        List<Vector3> ConvertNodeToV2(Stack<AStarPathNode> stackNode)
        {
        ...
        public List<Vector3> FindPath(Vector3 startPos, Vector3 targetPos, PathSize pathSize)
        {
            //// 리스트 초기화
            _openList.Clear();
            _closedList.Clear();
